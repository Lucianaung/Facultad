...
FFT fft;
int bands = 256;       //-- los lleve a presente
float[] spectrum;
boolean futuro = false;
presente presenteObj;
futuro futuroObj;
//-----------
...
void setup() {
 ...
 fullScreen();
  ...
  fondoa = minim.loadFile("estadoA.mp3");
  fondoa.setGain(-10.0);

  fondob = minim.loadFile("fondoB.mp3");
  fondob.setGain(-5.0);
  ...
  waveG = new Oscil(freParG[currentFreG], 0.01, Waves.SINE ); // GRAVE 0.2
  waveG.patch(outG); // patch the Oscil to the output

  waveM = new Oscil(freParM[currentFreM], 0.007, Waves.SINE ); // MEDIA 0.08
  waveM.patch(outM);

  //waveA = new Oscil(freParA[currentFreA], 0.04, Waves.SINE ); // AGUDA 0.04
  //waveA.patch(outA); //waveA.patch(adsr).patch(outA);

  waveB = new Oscil(freParB[currentFreB], 0.008, Waves.SINE ); // BASE  0.092
  waveB.patch(outB);

  //SOL-----
  fft = new FFT(fondoa.bufferSize(), fondoa.sampleRate());
  fft.linAverages(bands);
  spectrum = new float[bands];

  presenteObj = new presente();
  futuroObj = new futuro();
  //-------
...
void calculateSpectrumValues() {
  for (int i = 0; i < spectrum.length; i++) {
    spectrum[i] = fft.getAvg(i); // Obtener el valor promedio de cada banda
  }
}
